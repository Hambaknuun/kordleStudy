...
const Keyboard = ({ handleKeyboardClick, onChangeUserInput }) => {
    ...
    function generateKeyboard(arr) {
        let resultArr = [];
        arr.map((it, idx) => {
            resultArr.push(<KeyboardBtn val={it} clickVal={it} key={idx} className={"keyboardBtn"} onClickFunc={handleKeyboardClick} />)
        })
        return resultArr;
    }
    ...
    return (
        <article className="keyboard">
            ...
            <div className="keyboardLine">
                <KeyboardBtn val={'입력'} className={"keyboardFuncBtn"} onClickFunc={onChangeUserInput} clickVal={'Enter'} />
                {generateKeyboard(keyboardArr3)}
                <KeyboardBtn val={'삭제'} className={"keyboardFuncBtn"} onClickFunc={onChangeUserInput} clickVal={'Backspace'} />
            </div>
        </article>
    );
};
