...
export const setLocalGameStatistics = (haveWon, trial) => {
  ...
  if (haveWon) {
    ...
    const newGameStats = {
      ...localGameStats,
      bestStreak:
        localGameStats.bestStreak > localGameStats.currentStreak + 1
          ? localGameStats.bestStreak
          : localGameStats.currentStreak + 1,
      currentStreak: localGameStats.currentStreak + 1,
      totalGames: localGameStats.totalGames + 1,
      successRate:
        localGameStats.winDistribution.reduce((prev, now) => {
          return prev + now;
        }, 0) /
        (localGameStats.totalGames + 1),
      winDistribution: localGameStats.winDistribution.map((it, idx) => {
        if (idx === trial - 1) return it + 1;
        else return it;
      }),
    };
    ...
    const newGameStats = {
      ...localGameStats,
      gamesFailed: localGameStats.gamesFailed + 1,
      totalGames: localGameStats.totalGames + 1,
      successRate:
        localGameStats.winDistribution.reduce((prev, now) => {
          return prev + now;
...
const createLocalGameStatistics = () => {
  const gameStats = {
    bestStreak: 0,
    currentStreak: 0,
    gamesFailed: 0,
    bestsuccessRateStreak: 0,
    totalGames: 0,
